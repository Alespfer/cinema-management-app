...
/**
 * Implémentation concrète pour la gestion des avis clients dans le fichier
 * "evaluations_client.dat".
 *
 * Pour le développeur de l'interface graphique : cette classe est le moteur
 * derrière la section "Avis des spectateurs" de vos panneaux. - Quand un
 * utilisateur clique sur "Valider" dans la fenêtre `EvaluationDialog`, c'est la
 * méthode `addEvaluation` qui sera appelée en coulisses. - Pour afficher les
 * avis sur la page d'un film, vous utiliserez `getEvaluationsByFilmId`. - Pour
 * savoir s'il faut activer ou désactiver le bouton "Donner une note", vous
 * utiliserez `getEvaluationByClientAndFilm` pour vérifier si un avis existe
 * déjà.
 */
public class EvaluationClientDAOImpl extends GenericDAOImpl<EvaluationClient> implements EvaluationClientDAO {
    ...
    @Override
    public EvaluationClient getEvaluationByClientAndFilm(int clientId, int filmId) {
        for (EvaluationClient eval : this.data) {
