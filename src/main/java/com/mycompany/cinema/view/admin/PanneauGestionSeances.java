/*
 * PanneauGestionSeances.java
 * Panneau pour la gestion des séances de cinéma.
 * Il permet de créer, modifier et supprimer une séance en l'associant à un film et une salle.
 */
...
public class PanneauGestionSeances extends javax.swing.JPanel {
    ...
    public PanneauGestionSeances(AdminService adminService) {
    ...
    /**
     * Initialise les modèles de données et les moteurs de rendu.
     */
    ...
    /**
     * Charge la liste complète des séances depuis le service.
     */
    private void chargementSeances() {
        seanceListModel.clear();
        List<Seance> seances = adminService.trouverToutesLesSeances();
    ...
    /**
     * Charge les listes de films et de salles pour remplir les ComboBox.
     */
    ...
    /**
     * Méthode publique pour rafraîchir toutes les données du panneau.
     */
    ...
    /**
     * Affiche les détails d'une séance sélectionnée dans le formulaire.
     */
    private void afficherDetailsSeance(Seance seance) {
        if (seance != null) {
            jTextFieldDateHeure.setText(seance.getDateHeureDebut().format(FORMATTER));

            // Sélmection du bon film
    ...
    /**
     * Gère la logique de sauvegarde (création ou modification) d'une séance.
     */
    private void actionEnregistrer() {
        try {
            ...
            if (seanceSelectionnee == null) {
                int nouvelId = com.mycompany.cinema.util.IdManager.obtenirProchainIdSeance();
    ...
    /**
     * Gère la suppression de la séance sélectionnée.
     */
    ...
    private void jListSeancesValueChanged(javax.swing.event.ListSelectionEvent evt) {
        if (!evt.getValueIsAdjusting()) {
            ...
            afficherDetailsSeance(seanceSelectionnee);
        }
    }
