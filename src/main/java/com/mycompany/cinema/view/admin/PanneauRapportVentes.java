...
public class PanneauRapportVentes extends javax.swing.JPanel {
    ...
    public PanneauRapportVentes(AdminService adminService) {
    ...
    /**
     * Initialise les modèles de données pour les deux tableaux.
     */
    ...
    /**
     * Calcule et affiche le chiffre d'affaires pour une date spécifique.
     */
    private void actionCalculerCAJournalier() {
        ...
        try {
            ...
            // On met à jour les labels pour le CA journalier
    ...
    /**
     * Méthode principale de rafraîchissement, appelée au démarrage et par la
     * fenêtre principale.
     */
    ...
    /**
     * Calcule et affiche les chiffres d'affaires totaux (depuis le début) pour
     * les billets et les snacks.
     */
    private void calculerEtAfficherTotauxGeneraux() {
        double caTotalBillets = 0;
        List<Reservation> reservations = adminService.trouverToutesLesReservations();
        for (Reservation reservation : reservations) {
            List<Billet> billets = adminService.trouverBilletsParIdReservation(reservation.getId());
            for (Billet billet : billets) {
                Tarif tarif = adminService.trouverTarifParId(billet.getIdTarif());
                if (tarif != null) {
        ...
        List<VenteSnack> ventes = adminService.trouverToutesLesVentesSnack();
    ...
    /**
     * Charge et remplit le tableau avec le détail de toutes les réservations.
     */
    private void chargerTableauReservations() {
        reservationsTableModel.setRowCount(0);
        List<Reservation> reservations = adminService.trouverToutesLesReservations();
        for (Reservation reservation : reservations) {
            Client client = adminService.trouverClientParId(reservation.getIdClient());
            ...
            Seance seance = adminService.trouverSeanceParId(premierBillet.getIdSeance());
            ...
            if (seance != null) {
                ...
                Film film = adminService.trouverDetailsFilm(seance.getIdFilm());
                if (film != null) {
            ...
            for (Billet b : billets) {
                Tarif tarif = adminService.trouverTarifParId(b.getIdTarif());
                if (tarif != null) {
    ...
    /**
     * Charge et remplit le tableau avec le détail de toutes les ventes de
     * snacks.
     */
    private void chargerTableauVentesSnack() {
        ventesSnackTableModel.setRowCount(0);
        List<VenteSnack> ventes = adminService.trouverToutesLesVentesSnack();
        for (VenteSnack vente : ventes) {
            String canalDeVente, vendeur, clientAssocie;
            if (vente.getIdPersonnel() > 0) { // Si vente en caisse par un employé avec un ID valide
                Personnel p = adminService.trouverPersonnelParId(vente.getIdPersonnel());
                vendeur = (p != null) ? p.getPrenom() + " " + p.getNom() : "ID " + vente.getIdPersonnel();
                Caisse c = adminService.trouverCaisseParId(vente.getIdCaisse());
                canalDeVente = (c != null) ? c.getNom() : "Caisse ID " + vente.getIdCaisse();
            } else { // Si vente en ligne
                vendeur = "N/A";
                canalDeVente = "Achat en Ligne";
            }
            if (vente.getIdClient() != null) {
                Client c = adminService.trouverClientParId(vente.getIdClient());
