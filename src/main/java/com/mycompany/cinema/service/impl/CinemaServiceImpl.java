 ...
 * Cette classe unique implémente TOUTES les règles de gestion et les fonctionnalités,
 * que ce soit pour les clients ou pour les administrateurs.
 *
 * Pour le développeur de l'interface graphique : ce fichier est votre UNIQUE POINT DE CONTACT
 * avec la logique de l'application. Tous vos panneaux, fenêtres et dialogues n'appelleront
 * que des méthodes de cette classe (via les interfaces ClientService ou AdminService).
 *
 * Elle agit comme une "façade" : elle vous offre des méthodes simples (ex: "réserver un billet")
 * et se charge de toute la complexité en coulisses (vérifier la disponibilité,
 * créer les objets, les sauvegarder, etc.).
 */
public class CinemaServiceImpl implements ClientService, AdminService {
    ...
    /**
     * Méthode utilitaire pour forcer toutes les couches de données à se rafraîchir.
     * Utile pour garantir que l'interface affiche les données les plus à jour après
     * une modification majeure (ex: un admin ajoute un film, un autre panneau doit le voir).
     */
    @Override
    ...
    /**
     * Récupère la liste de tous les films à l'affiche.
     * Utilisé par le ProgrammationPanel pour l'affichage initial.
     */
    @Override
    public List<Film> getFilmsAffiche() { return filmDAO.getAllFilms(); }
    
    /**
     * Récupère toutes les informations d'un film spécifique par son ID.
     * Utilisé par le FilmDetailPanel.
     */
    @Override
    public Film getFilmDetails(int filmId) { return filmDAO.getFilmById(filmId).orElse(null); }

    /**
     * Recherche des films dont le titre contient un certain mot-clé.
     * Utilisé par la barre de recherche du ProgrammationPanel.
     */
    @Override
    public List<Film> findFilmsByTitre(String keyword) { return filmDAO.findFilmsByTitre(keyword); }
    
    /**
     * Calcule et retourne la note moyenne donnée par les spectateurs pour un film.
     * Utilisé par le FilmDetailPanel pour afficher la note à côté de celle de la presse.
     */
    @Override
     ...
     * Applique la règle métier : un email ne peut pas être utilisé deux fois.
     * @return Le nouvel objet Client créé.
     * @throws Exception Si l'email existe déjà. La Vue devra intercepter cette exception et afficher un message d'erreur.
     */
    @Override
     ...
     * @return Un Optional contenant le Client s'il est trouvé, sinon un Optional vide. La Vue vérifiera si le Optional est vide ou non.
     */
    @Override
    ...
    /**
     * Met à jour les informations d'un client (nom, mot de passe).
     * Utilisé par le InfosPersonnellesPanel.
     */
    @Override
     ...
     * Règle de gestion : annule d'abord toutes ses réservations existantes.
     */
    @Override
    ...
    /**
     * Logique de recherche avancée pour le ProgrammationPanel, combinant plusieurs filtres.
     * C'est un bon exemple d'orchestration complexe.
     */
    @Override
    ...
    /**
     * Logique métier pour déterminer les sièges encore disponibles pour une séance.
     * C'est une orchestration clé pour le SiegePanel.
     */
    @Override
    ...
    /**
     * Gère la transaction de réservation de billets.
     * Applique plusieurs règles de gestion avant de contacter les DAO.
     */
    @Override
     ...
     * C'est la méthode la plus complexe car elle doit garantir que soit TOUT réussit,
     * soit TOUT est annulé, pour éviter des incohérences de données (ex: billets réservés
     * mais pas assez de stock pour le popcorn).
     *
     ...
     * @throws Exception Si une étape échoue (siège déjà pris, stock insuffisant...).
     */
    @Override
    ...
    /**
     * Tente d'authentifier un membre du personnel.
     * Utilisé par la LoginFrame pour l'accès au back-office.
     */
    @Override
     ...
     * Applique une règle de gestion essentielle : un produit ne peut être supprimé
     * que s'il n'est référencé dans AUCUNE vente passée. Cela garantit
     * l'intégrité des rapports de ventes historiques.
     *
     * @param pId L'ID du produit à supprimer.
     * @throws Exception Si le produit est trouvé dans une ou plusieurs lignes de vente,
    ...
    public void supprimerProduitSnack(int pId) throws Exception {
     ...
     /**
     * Gère la transaction complète d'une vente de snacks effectuée au point de vente.
     * Cette méthode applique plusieurs règles de gestion critiques :
     * 1. Vérifie la validité de l'employé, de la caisse et du panier.
     * 2. Vérifie que le stock est suffisant pour TOUS les articles du panier AVANT de commencer la transaction.
     * 3. Si tout est valide, elle crée la vente, crée chaque ligne de détail, et met à jour les stocks.
     *
     ...
     * @return L'objet VenteSnack créé, contenant l'ID et la date de la transaction.
     * @throws Exception Si une règle de gestion est violée (ex: stock insuffisant), une exception est levée pour annuler toute l'opération.
     */
    @Override
